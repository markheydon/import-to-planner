...
**Constraints**: Fluent UI Blazor must remain the sole UI library; custom CSS must be minimal and use Fluent design tokens where possible  
...
- **Testing Gate**: ✅ Existing `HomePageWorkflowTests.cs` and `HomePageSmokeTests.cs` (bUnit) must pass without modification to test logic. New tests required for: step activation sequence, stale-preview warning placement, FluentTabs content in execution report. Test coverage for step-state transitions uses in-memory gateway mode.
...
- Existing `@code { }` block logic in `Home.razor` must be preserved in full with no functional changes. The redesign touches **markup only** plus scoped CSS.
