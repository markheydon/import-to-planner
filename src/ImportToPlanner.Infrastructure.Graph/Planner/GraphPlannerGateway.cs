...
public sealed class GraphPlannerGateway : IPlannerGateway
{
        ...
        while (bucketsResponse is not null)
        {
            if (bucketsResponse.Value is not null)
            {
                buckets.AddRange(bucketsResponse.Value
                    ...
                    .Select(bucket => MapPlannerBucket(bucket, planId)));
            }
    ...
    public async Task<PlannerBucket> CreateBucketAsync(string planId, string bucketName, CancellationToken cancellationToken)
    {
        ...
        return MapPlannerBucket(created, planId);
    }
        ...
        while (tasksResponse is not null)
        {
            if (tasksResponse.Value is not null)
            {
                tasks.AddRange(tasksResponse.Value
                    ...
                    .Select(task => MapPlannerTaskSnapshot(task, planId)));
            }
    ...
    private static PlannerBucket MapPlannerBucket(GraphPlannerBucket graphBucket, string fallbackPlanId)
    {
        ArgumentNullException.ThrowIfNull(graphBucket);
        ValidateRequired(fallbackPlanId, nameof(fallbackPlanId));

        if (string.IsNullOrWhiteSpace(graphBucket.Id) || string.IsNullOrWhiteSpace(graphBucket.Name))
        {
            throw new InvalidOperationException("Graph returned an invalid bucket response.");
        }

        return new PlannerBucket(graphBucket.Id, graphBucket.Name, graphBucket.PlanId ?? fallbackPlanId);
    }

    private static PlannerTaskSnapshot MapPlannerTaskSnapshot(GraphPlannerTask graphTask, string fallbackPlanId)
    {
        ArgumentNullException.ThrowIfNull(graphTask);
        ValidateRequired(fallbackPlanId, nameof(fallbackPlanId));

        if (string.IsNullOrWhiteSpace(graphTask.Id) || string.IsNullOrWhiteSpace(graphTask.Title))
        {
            throw new InvalidOperationException("Graph returned an invalid task response.");
        }

        return new PlannerTaskSnapshot(graphTask.Id, graphTask.Title, graphTask.PlanId ?? fallbackPlanId);
    }
