...
- This file applies to `*.razor.cs`, but Blazor component-facing decisions in those files are governed first by the Blazor UI instruction file (`blazor-mudblazor.instructions.md`, Fluent UI oriented).
...
For `*.razor.cs`, Blazor component-facing concerns are governed first by `blazor-mudblazor.instructions.md` (Fluent UI oriented), while Clean Architecture governs non-UI policy boundaries and dependency direction.