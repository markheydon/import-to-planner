...
- Ensure CI keeps validating the current scope: `dotnet restore ImportToPlanner.slnx`, `dotnet format ImportToPlanner.slnx --no-restore --verify-no-changes --verbosity minimal`, `dotnet build ImportToPlanner.slnx --no-restore`, and `dotnet test ImportToPlanner.slnx --no-build`. AppHost projects are validated via solution restore/build; AppHost orchestration is not tested.
